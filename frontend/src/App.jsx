...
const styles = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tab:hover { color: ${COLORS.textPrimary}; background: #1a1a1a; }
  .tab.active { color: ${COLORS.textPrimary}; background: #1e1e1e; }
  ...
  .ghost-btn:hover { border-color: ${COLORS.borderHover}; color: ${COLORS.textPrimary}; background: #141414; }
...
const Icons = {
  ...
  billing: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-4H9l3-3 3 3h-2v4z M9 9V7M15 9V7",
...
const StatCard = ({ label, value, sub, accent, loading, delay = 0 }) => (
  <div className={`card fade-up-${delay + 1}`} style={{ padding: "20px 24px" }}>
    ...
    {loading
      ...
      : <p className="stat-number" style={{ color: accent || COLORS.textPrimary, marginBottom: 6 }}>{value}</p>
    }
    {loading
      ? <div className="skeleton" style={{ height: 14, width: "40%" }} />
      : <p style={{ color: COLORS.textSecondary, fontSize: 12 }}>{sub}</p>
    }
  </div>
);
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: "28px 20px 24px", borderBottom: `1px solid ${COLORS.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            background: `linear-gradient(135deg, ${COLORS.accent}, #00bfff)`,
          ...
          <div>
            <p style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 15, letterSpacing: "-0.01em" }}>MeterFlow</p>
      ...
      <nav style={{ padding: "16px 12px", flex: 1 }}>
        ...
        {links.map(link => (
          <button key={link.id} className={`nav-link ${active === link.id ? "active" : ""}`} onClick={() => setActive(link.id)}>
            ...
            {link.label}
            {active === link.id && (
              <span style={{ marginLeft: "auto", width: 4, height: 4, borderRadius: "50%", background: COLORS.accent }} />
            )}
          </button>
        ))}
      </nav>
      ...
      <div style={{ padding: "16px 20px", borderTop: `1px solid ${COLORS.border}` }}>
        <p style={{ fontSize: 11, color: COLORS.textTertiary }}>v1.0.0 · FastAPI + MongoDB</p>
      </div>
    </aside>
  );
};
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 26, letterSpacing: "-0.02em", marginBottom: 4 }}>
          Overview
        </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
        ...
        <StatCard label="Avg Response" value={data ? `${data.avg_response_time_ms?.toFixed(0)}ms` : "—"} sub="Last 100 requests" loading={loading} delay={1} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16, marginBottom: 16 }}>
        ...
        <div className="card fade-up-5" style={{ padding: "24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div>
              <p style={{ fontWeight: 500, fontSize: 14, marginBottom: 2 }}>Request Volume</p>
              <p style={{ color: COLORS.textSecondary, fontSize: 12 }}>Simulated 24h trend</p>
            </div>
            <span style={{ fontSize: 11, color: COLORS.textSecondary, fontFamily: COLORS.mono }}>24h</span>
          </div>
        ...
        <div className="card fade-up-5" style={{ padding: "24px" }}>
          ...
          {chartData.length > 0 ? (
              ...
              <ResponsiveContainer width="100%" height={140}>
                <PieChart>
                  <Pie data={chartData} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" strokeWidth={0}>
                    {chartData.map((_, i) => (
                      <Cell key={i} fill={[COLORS.accent, COLORS.blue, COLORS.amber, "#a78bfa"][i % 4]} />
                    ))}
                  </Pie>
                  <Tooltip content={<CustomTooltip />} />
                </PieChart>
            ...
            <div style={{ height: 140, display: "flex", alignItems: "center", justifyContent: "center", color: COLORS.textTertiary, fontSize: 13 }}>
              No endpoint data
            </div>
          )}
        </div>
...
const ApiKeys = ({ notify }) => {
  ...
  const generateKey = async () => {
    ...
    try {
      ...
      notify("Failed to generate key — is the server running?", "error");
    }
    setLoading(false);
  };
  ...
  return (
    <div>
        ...
        {generatedKey && (
          <div style={{
            ...
            <div>
              ...
              <code style={{ fontFamily: COLORS.mono, fontSize: 13, color: COLORS.textPrimary, letterSpacing: "0.04em" }}>{generatedKey}</code>
            </div>
      ...
      <div className="card fade-up-2">
        ...
        {keys.length === 0 ? (
          <div style={{ padding: "48px", textAlign: "center", color: COLORS.textTertiary, fontSize: 13 }}>
            No keys generated yet
          </div>
        ) : (
          <div>
            {/* Header */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 160px 40px", padding: "10px 24px", borderBottom: `1px solid ${COLORS.border}` }}>
              {["Key", "Plan", "Created", ""].map(h => (
                <span key={h} style={{ fontSize: 11, color: COLORS.textTertiary, textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>
              ))}
            </div>
            {keys.map((k, i) => (
              <div key={i} style={{
                display: "grid", gridTemplateColumns: "1fr 80px 160px 40px",
                padding: "14px 24px", borderBottom: i < keys.length - 1 ? `1px solid ${COLORS.border}` : "none",
                alignItems: "center",
              }}>
                <span className="key-badge">{k.key}</span>
                <span className={`plan-badge plan-${k.plan}`}>{k.plan}</span>
                <span style={{ fontSize: 12, color: COLORS.textSecondary, fontFamily: COLORS.mono }}>
                  {new Date(k.created).toLocaleTimeString()}
                </span>
                <button className="copy-btn" onClick={() => copyKey(k.key)}>
...
const Analytics = ({ notify }) => {
  ...
  const fetch_ = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/analytics/${apiKey}`, {
        headers: { "x-api-key": apiKey },
      });
      const json = await res.json();
      setData(json);
    } catch {
      notify("Failed to fetch analytics", "error");
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      ...
      <div className="card fade-up-1" style={{ padding: "20px 24px", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <input
            ...
            style={{ fontFamily: COLORS.mono, fontSize: 12 }}
          />
              ...
              <BarChart data={endpointData} barSize={32}>
                ...
                <Bar dataKey="count" name="Requests" radius={[4, 4, 0, 0]}>
                  {endpointData.map((_, i) => (
                    <Cell key={i} fill={i === 0 ? COLORS.accent : COLORS.blue} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      {data?.error && (
        <div className="card fade-up-2" style={{ padding: "32px", textAlign: "center", color: COLORS.red, fontSize: 13 }}>
          <Icon d={Icons.alert} size={20} color={COLORS.red} />
          <p style={{ marginTop: 8 }}>{data.error}</p>
        </div>
      )}
    </div>
  );
...
const Billing = ({ notify }) => {
  ...
  const fetchBilling = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/billing/${userId}`, {
        headers: { "x-api-key": apiKey },
      });
      const json = await res.json();
      setData(json);
    } catch {
      notify("Failed to fetch billing data", "error");
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      ...
      <div className="card fade-up-2" style={{ padding: "24px", marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {Object.entries(PRICING).map(([plan, price]) => (
            <div key={plan} style={{
              padding: "16px 20px",
              background: plan === "pro" ? COLORS.blueDim : "#0a0a0a",
              border: `1px solid ${plan === "pro" ? COLORS.blue + "44" : COLORS.border}`,
              borderRadius: 10,
            }}>
        ...
        <div className="card fade-up-3">
          <div style={{ padding: "20px 24px", borderBottom: `1px solid ${COLORS.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <p style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 22, color: COLORS.accent }}>
          ...
          {data.keys?.map((k, i) => (
            <div key={i} style={{
              ...
              padding: "14px 24px",
              borderBottom: i < data.keys.length - 1 ? `1px solid ${COLORS.border}` : "none",
              alignItems: "center",
            }}>
      ...
      {data?.error && (
        <div className="card fade-up-3" style={{ padding: "32px", textAlign: "center", color: COLORS.textSecondary, fontSize: 13 }}>
          {data.error}
        </div>
      )}
    </div>
...
export default function App() {
  ...
  // Fetch overview analytics using the stored key
  useEffect(() => {
    const key = localStorage.getItem("mf_api_key");
    if (!key || page !== "overview") return;
    setAnalyticsLoading(true);
    fetch(`${API_BASE}/analytics/${key}`, { headers: { "x-api-key": key } })
      .then(r => r.json())
    ...
    <>
      ...
      <div style={{ display: "flex" }}>
        ...
        <main style={{ marginLeft: 220, flex: 1, padding: "40px 48px", maxWidth: "calc(100vw - 220px)", minHeight: "100vh" }}>
          ...
          {page === "overview" && <Overview data={analyticsData} loading={analyticsLoading} />}
